# Store Page Detail Slideshow Item Badge Add

### [License][1]

### Synopsis

Add badges to product detail slideshow badges.

### Version

  * 0.3.0

#### SS Versions

  * 7.1
  
  * 7.0 ([Brine template family][2])

#### v7.1 Products V2 Compatible

  * Yes
  
#### 7.1 Fluid Engine Compatible

  * Not Applicable

#### Dependencies

  * [Squarespace plan][3] that supports [JavaScript][4].

---

## Quick Install

* Options

  * Page Specific
  
    * Use this option if you have only a single store page you want to have this
      effect.
      
    * Add code from file **[store page detail slideshow item badge
      add.html][14]** to Store Page Settings > Advanced >
      Page Header Code Injection for the Page.
      
    * Refer to [per-page code injection][5] for details.
      
  * Site-wide
  
    * Use this option if you have multiple store pages where you want to have
      this effect.
      
    * v7.1
    
      * Add code from file **[store page detail slideshow item badge
        add.html][14]** to Website > Pages > Custom Code > Code Injection >
        FOOTER.
        
    * v7.0
    
      * Add code from file **[store page detail slideshow item badge
        add.html][14]** to Website > Pages > Website Tools > Custom Code >
        Code Injection > FOOTER.
        
    * Refer to [Add code to code injection][6].

## Make a Donation

Please consider [making a donation][8].

## Changes

* **2025-11-17**

  * updated to work with v7.1 Products V2
  * removed twcsl dependency
  * removed jQuery dependency
  * bumped version to 0.3.0
  
* **2023-03-27**

  * add support for v7.1 product detail layouts
  * bumped version to v0.2.0
  
* **2021-12-22**

  * verified code works with v7.1, this code was based on code I had done for it
    previously but used a different method to gather the badge text
  * bumped version to v0.1d2
  
* **2021-12-22**

  * initial version

[1]: https://github.com/tomsWebConsulting/twcsl/blob/main/LICENSE.txt#L1
[2]: https://support.squarespace.com/hc/en-us/articles/212512738-Brine-template-family
[3]: https://www.squarespace.com/pricing
[4]: https://en.wikipedia.org/wiki/JavaScript
[14]: store%20page%20detail%20slideshow%20item%20badge%20add.html#L1
[5]: https://support.squarespace.com/hc/en-us/articles/205815908-Using-code-injection#h_01JGPDM34K6435FJV3FQSBAE7X
[6]: https://support.squarespace.com/hc/en-us/articles/205815908-Using-code-injection#h_01JGPDM34K9B0J2SNGJE936M7K
[7]: https://support.squarespace.com/hc/en-us/articles/205815908-Using-code-injection#h_01JGPDM34K6435FJV3FQSBAE7X
[8]: https://github.com/tomsWebConsulting/twcsl#make-a-donation
